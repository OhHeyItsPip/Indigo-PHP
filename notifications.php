...
4=Comment on Post 
*/
$notifs = $db->query("SELECT * FROM notifs WHERE notif_to = '".$_SESSION['user_name']."' ORDER BY id DESC LIMIT 50");
$notif_count = mysqli_num_rows($notifs);
?>

<html>
	<head>
		<?php formHeaders('Updates - Indigo'); ?>
	</head>
	<body>
		<div id="wrapper">
			<div id="sub-body">
				<?php 
				echo form_top_bar($_SESSION['user_name'], $_SESSION['user_id'], $_SESSION['user_face'], 'updates');
				?>
			</div>			
			<div id="main-body">
				<div class="main-column messages">
					<div class="post-list-outline">
						...
						<div class="list news-list">
							<?php 
								if($notif_count == 0) {
									echo '<div class="no-content"><div><p>There does not appear to be any updates. Try making posts and discuss your favorite games!</p></div><img src="/assets/img/group.png" alt="Groupies"></div>';
								} else {
									while($updates = mysqli_fetch_array($notifs)) {
										$users = $db->query("SELECT * FROM users WHERE user_name = '".$updates['notif_by']."' ");
...
<?php 
$db->query("UPDATE notifs SET rd_notif = 1 WHERE notif_to = '".$_SESSION['user_name']."'");
?>