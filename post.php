...
include('lib/htm.php');
if(account_deleted($_SESSION['user_id'])) {
...
$a = "SELECT * FROM titles WHERE id = ".$_POST['title']." AND deleted = 0";
$b = mysqli_query($db,$a);
$d = mysqli_fetch_array($b);
$c = mysqli_num_rows($b);
if($c !== 1) {
	$err = "The hub you're trying to post in doesn't exist.";
}
if($d['rd_oly'] == 'true' && $user['user_level'] < 6) {
	$err = "You can't post in this Hub.";
}
...
if(!isset($err)) {
$sql_post = "INSERT INTO posts (id, text, owner, title, screenshot, web_url, feeling) VALUES ('$cid', '$content', '$cvid', '$ip', '$screenshot', '$url', '$feeling')";
