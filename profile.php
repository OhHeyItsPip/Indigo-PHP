...
session_start();
if($_GET['page'] == 1) {
  $redirect = '/users/'.$_GET['user_name'];
} elseif ($_GET['page'] == 3) {
    $redirect = '/users/'.$_GET['user_name'].'/replies';
} elseif($_GET['page'] == 2) {
    $redirect = '/users/'.$_GET['user_name'].'/empathies';
} elseif($_GET['page'] == 4) {
    $redirect = '/users/'.$_GET['user_name'].'/deleted';
} elseif($_GET['page'] == 5) {
    $redirect = '/users/'.$_GET['user_name'].'/followers';
} elseif($_GET['page'] == 6) {
    $redirect = '/users/'.$_GET['user_name'].'/following';
}
Require('lib/connect.php');
include('lib/htm.php');
include('lib/users.php');
$userid = mysqli_real_escape_string($db,$_GET['user_name']);
$page = mysqli_real_escape_string($db,$_GET['page']);
$sql = "SELECT nickname,user_level FROM users WHERE user_name= '$userid'";
$pfp = user_pfp($userid,0);
$result = $db->query($sql);
$row = $result->fetch_assoc();
$count = mysqli_num_rows($result);
if($count !== 1) {
die('<p>That user does not exist. Did you make a spelling error? Were they deleted?</p>

<img src="/assets/img/yellowerror.png" alt="Yellow Error Guy">
...
$chk = $db->query("SELECT * FROM follows WHERE follow_to = '$userid' AND follow_by = '".$_SESSION['user_name']."'");
...
if($page == 4 & $userid != $_SESSION['user_name']) {
  exit('<p>You\'re not aloud to view this page.<p>

<img src="/assets/img/pinkerror.png" alt="Pink Error Girl">'
);
}
?>
...
<html>
  <head>
    ...
    formHeaders($row['nickname'].'\'s posts - Indigo');
    }else{
    formHeaders('Your posts - Indigo');
    }
    }elseif($page == 3) {
    if($_SESSION['user_name'] !== $_GET['user_name']) {
    formHeaders($row['nickname'].'\'s replies - Indigo');
    }else{
    formHeaders('Your replies - Indigo');
    }
    }elseif($page == 2) {
    if($_SESSION['user_name'] !== $_GET['user_name']) {
    formHeaders($row['nickname'].'\'s yeahs - Indigo');
    }else{
    formHeaders('Your yeahs - Indigo');
    }
    }elseif($page == 4) {
    formHeaders('Your deleted posts - Indigo');
    }elseif($page == 5) {
    if($_SESSION['user_name'] !== $_GET['user_name']) {
    formHeaders($row['nickname'].'\'s followers - Indigo');
    }else{
    formHeaders('Your followers - Indigo');
    }
    }elseif($page == 6) {
    if($_SESSION['user_name'] !== $_GET['user_name']) {
    formHeaders($row['nickname'].'\'s following - Indigo');
    }else{
    formHeaders('Your following - Indigo');
    }
    }
    } else {
    if($page == 1) {
  ...
  echo form_top_bar($_SESSION['user_name'], $_SESSION['user_id'], $_SESSION['user_face'], 'user');
  } else { 
  echo ftbnli('user'); }
         ?>
      </div>
    ...
    <div id="sidebar-profile-body" class="without-profile-post-image">
      ...
      <p class="id-name"><?php echo $userid; ?></p>
       ...
       <?php if(isset($_SESSION['user_name']) && $_SESSION['user_name'] == $userid) { echo '<div id="edit-profile-settings"><a class="button symbol" href="/edit/profile">Edit Profile</a></div>'; }
        if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == 'true') {
        if($userid !== $_SESSION['user_name']) {
          if($following_user == 0) {
   ...
   <div class="post-list-outline">
      ...
      endwhile;
             ...
             while($post_row = mysqli_fetch_array($comments)): 
                        if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
          if($post_row['owner'] == $_SESSION['user_name']) {
            $yeah_disabled = true;
          } else {
            $yeah_disabled = false;
          }
        ...
        if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
        $check_yeah_added = $db->query("SELECT * FROM yeahs WHERE owner = '".$_SESSION['user_name']."' AND post_id = ".$post_row['id']." AND type = 'comment' ");
    ...
    <div class="post-content">
      ...
      endwhile;
        ...
        if($yeah_count == 0) { echo "This user has no yeahs yet."; } else {
          ...
          if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
          if($post['owner'] == $_SESSION['user_name']) {
            $yeah_disabled = true;
          } else {
            $yeah_disabled = false;
          }
        ...
        if(mysqli_num_rows($query) !== 0) {
          printPost($post['post_id'],1);
              ...
              $query = $db->query("SELECT comments.id, comments.post_id, comments.yeahs, comments.content, comments.owner, users.pfp, users.nickname, users.user_type, comments.date_time, comments.feeling FROM comments, users WHERE comments.id = ".$post_row['post_id']." AND users.user_name = comments.owner");
              $post = mysqli_fetch_array($query);
                        if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
                          if($post['owner'] == $_SESSION['user_name']) {
                            $yeah_disabled = true;
              ...
              if(mysqli_num_rows($query) !== 0) {
                if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
                  $check_yeah_added = $db->query("SELECT * FROM yeahs WHERE owner = '".$_SESSION['user_name']."' AND post_id =".$post['id']." AND type = 'comment' ");
                  $yeahed = mysqli_num_rows($check_yeah_added);
                } else {
                  $yeahed = 0;
                }
      ...
      endwhile;
        ...
        if($follower_count == 0) { echo "This user doesn't have any followers yet."; } else {
          ...
          while($post_row = mysqli_fetch_array($followers)):
            $get_u = $db->query("SELECT * FROM users WHERE user_name = '".$post_row['follow_by']."'");
            $users = mysqli_fetch_array($get_u);
            echo '
            <li class="trigger" data-href="/users/'.$users['user_name'].'">
          <a href="/users/'.$users['user_name'].'" class="icon-container '.($users['user_type'] > 2 ? 'official-user' : '').'">
            <img src="'.user_pfp($users['user_name'],0).'" class="icon">
          </a>
            <div class="toggle-button"></div>
          <div class="body">
            <p class="title">
              <span class="nick-name">
                <a href="/users/'.$users['user_name'].'">'.$users['nickname'].'</a>
              </span>
        ...
        if($following_count == 0) { echo "This user didn't follow anyone yet."; } else {
          ...
          while($post_row = mysqli_fetch_array($following)):
            $get_u = $db->query("SELECT * FROM users WHERE user_id = '".$post_row['follow_to']."'");
            $users = mysqli_fetch_array($get_u);
            echo '
            <li class="trigger" data-href="/users/'.$users['user_id'].'">
          <a href="/users/'.$users['user_name'].'" class="icon-container '.($users['user_type'] > 2 ? 'official-user' : '').'">
            <img src="'.user_pfp($users['user_id'],0).'" class="icon">
          </a>
            <div class="toggle-button"></div>
          <div class="body">
            <p class="title">
